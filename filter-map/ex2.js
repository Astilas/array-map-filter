...
function getCampusesTeachingReact(campuses) {
  const result = campuses.filter(function(campusJS){
    for (let i=0; i<=campusJS.curriculums.length;i++){
      if (campusJS.curriculums[i] === 'JS/React')
        return campusJS.curriculums[i];
    }
  })
