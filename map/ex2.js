...
function getFoodCategories(foods) {
  const a = foods.map(function(qqchose){
    if (qqchose.isVegetarian=== true){
      return `${qqchose.food} is suitable for vegetarians`;
    } else {
      return `${qqchose.food} is not suitable for vegetarians`;
    }
